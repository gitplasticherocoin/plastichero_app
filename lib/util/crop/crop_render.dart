...
class CropRenderObjectWidget extends SingleChildRenderObjectWidget {
  final String tag = 'CropRenderObjectWidget';
...
class RenderCrop extends RenderBox with RenderObjectWithChildMixin<RenderBox> {
  ...
  void performLayout() {
    ...
    if (child != null) {
      final forcedSize = _getSizeToFitByRatio(aspectRatio!, size.width, size.height);
  ...
  Path _getDimClipPath() {
    ...
    final forcedSize = _getSizeToFitByRatio(aspectRatio!, size.width, size.height);
    Rect rect = Rect.fromCenter(center: center, width: forcedSize.width, height: forcedSize.height);
...
Size _getSizeToFitByRatio(double imageAspectRatio, double containerWidth, double containerHeight) {
